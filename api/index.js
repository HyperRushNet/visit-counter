...
export default function handler(req, res) {
  ...
  // **Direct response naar gebruiker – zeer snel**
  res.status(200).json({ message: '+1 scheduled - Counter' });

  // **Background fetch, verhoogt echt teller**
  (async () => {
    try {
      global._pendingFetch = global._pendingFetch || false;

      // Voorkom dubbele fetches tegelijk
      if (!global._pendingFetch) {
        global._pendingFetch = true;

        await fetch(`${scriptUrl}?ua=vercel-fetch&mod=0`, {
